 ...
 * (e.g. network, terminal or test environment)
 */
trait EngineIO {
...
class GameEngine(io: EngineIO, initialPlayers: List[Player]) {
  ...
  private def gameLoop(currentPlayers: List[Player], dealerIndex: Int, round: Int): Unit = {
    ...
    announceBustedAfterHand(players, updatedPlayers)
  ...
  private def announceBustedAfterHand(before: List[Player], after: List[Player]): Unit = {
    before.zip(after).foreach { case (prev, curr) =>
      if (prev.chips > 0 && curr.chips <= 0) {
        io.broadcast(s"\n${curr.name} has no chips left and is removed from the table.")
      }
    }
  }
  ...
  private def playRound(allPlayers: List[Player], dealerIdx: Int): List[Player] = {
    ...
    def prepareBlinds(ps: List[Player]): (List[Player], List[Int], Int, Int) = {
      val eligibleSeats = seatOrder.filter(i => ps(i).chips > 0)
      if (eligibleSeats.size < 2) return (ps, eligibleSeats, -1, -1)
      ...
      (ps, eligibleSeats, sbSeat, bbSeat)
    }

    val (preparedPlayers, eligibleSeats, sbSeat, bbSeat) = prepareBlinds(freshPlayers)
    if (eligibleSeats.size < 2) return preparedPlayers

    var players = preparedPlayers.zipWithIndex.map {
      case (p, i) =>
        if (i == sbSeat) p.removeChips(SmallBlind)
        else if (i == bbSeat) p.removeChips(BigBlind)
        else p
    }

    val initialBets = Array.fill(n)(0)
    initialBets(sbSeat) = SmallBlind
    initialBets(bbSeat) = BigBlind

    var pot = initialBets.sum
    io.broadcast(s"\nBlinds: ${preparedPlayers(sbSeat).name} posts SB $SmallBlind, ${preparedPlayers(bbSeat).name} posts BB $BigBlind | Pot: $pot")
    ...
    // Pre-flop
    val (ps1, pot1, history1, ended1) =
      bettingRound(players, eligibleSeats, pot, BigBlind, initialBets.clone(), utg, Nil, Street.PreFlop, history, seatPositions)
    players = ps1
    pot = pot1
    history = history1
    ...
    // Flop
    val (flop, d2) = deck.deal(3)
    deck = d2
    val community1 = flop
    io.broadcast(s"\n── Flop ── Community: ${showCards(community1)}")
    showHoleCards(players, eligibleSeats)

    val (ps2, pot2, history2, ended2) =
      bettingRound(players, eligibleSeats, pot, 0, Array.fill(n)(0), 0, community1, Street.Flop, history, seatPositions)
    players = ps2
    pot = pot2
    history = history2
    ...
    // Turn
    val (turn, d3) = deck.deal(1)
    deck = d3
    val community2 = community1 ++ turn
    io.broadcast(s"\n── Turn ── Community: ${showCards(community2)}")
    showHoleCards(players, eligibleSeats)

    val (ps3, pot3, history3, ended3) =
      bettingRound(players, eligibleSeats, pot, 0, Array.fill(n)(0), 0, community2, Street.Turn, history, seatPositions)
    players = ps3
    pot = pot3
    history = history3
    ...
    val (ps4, pot4, _, ended4) =
      bettingRound(players, eligibleSeats, pot, 0, Array.fill(n)(0), 0, community3, Street.River, history, seatPositions)
    players = ps4
    pot = pot4
  ...
  private def orderedSeatsFrom(
      eligibleSeats: List[Int],
      startSeat: Int,
      includeStart: Boolean,
      players: List[Player]
  ): List[Int] = {
    val startIdx = eligibleSeats.indexOf(startSeat)
    if (startIdx < 0 || eligibleSeats.isEmpty) return Nil

    val offset = if (includeStart) 0 else 1
    (offset until (eligibleSeats.size + offset))
      .map(k => eligibleSeats((startIdx + k) % eligibleSeats.size))
      .filter(i => !players(i).folded && players(i).chips > 0)
      .toList
  }
  ...
  private def bettingRound(
    ...
    if (numSeats == 0) return (ps, potV, historyV, false)

    val firstSeat = eligibleSeats(Math.floorMod(firstActIdx, numSeats))
    var pending = orderedSeatsFrom(eligibleSeats, firstSeat, includeStart = true, ps)

    // Nobody can make a decision on this street -> go straight to next street / showdown
    if (pending.isEmpty) return (ps, potV, historyV, false)

    while (pending.nonEmpty) {
      if (!io.isGameActive) return (ps, potV, historyV, true)

      val seatI = pending.head
      val player = ps(seatI)
      ...
      if (player.folded || player.chips <= 0) {
        ...
        val action = player.playerType match {
          ...
          case PlayerType.Computer =>
            val ctx = GameContext(
              ...
              player.chips,
              currentBetV,
              activePlayers(ps, eligibleSeats).size,
              activePlayers(ps, eligibleSeats).filterNot(i => ps(i).name == player.name).map(i => ps(i).name -> ps(i).chips),
              seatPositions.getOrElse(seatI, "Unknown"),
              activePlayers(ps, eligibleSeats).filterNot(i => ps(i).name == player.name).map(i => ps(i).name -> seatPositions.getOrElse(i, "Unknown")).toMap,
              historyV
            )
        ...
        resolvedAction match {
          case Action.Raise(_) =>
            pending = orderedSeatsFrom(eligibleSeats, seatI, includeStart = false, ps)
          case _ =>
            pending = pending.tail
        }

        // If nobody on this street can still act, stop here and move on.
        if (pending.isEmpty || pending.forall(i => ps(i).chips <= 0)) {
          return (ps, potV, historyV, false)
        }
      }
    }
